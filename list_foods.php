...
# grab all of the users saved foods
$sql = sprintf ("
	SELECT userFoods.*
	FROM userFoods INNER JOIN users
		ON userFoods.user = users.id
	WHERE users.id = '%s'
	ORDER BY userFoods.description
	",
	$_SESSION['user']['id']
);
$db->Select($sql);
if ( $db->_rowCount > 0 ) {
	$smarty->assign("foodCount", $db->_rowCount);
	$smarty->assign("userFoods", $db->_rows);
}
